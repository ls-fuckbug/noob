...
当编码为跳表时，一个zset结构同时包含一个字典和一个跳跃表
...
哈希桶中的元素保存的并不是值本身，而是指向具体值的指针。这也就是说，不管值是String，还是集合类型，哈希桶中的元素都是指向它们的指针。实际上就是redis Object类型的指针。
...
4. 二进制安全 （C语言中，用'\0'表示字符串的结束，如果字符串中本身就有'\0'字符，字符串就会被截断，即非二进制安全）
5. 以\0结尾，兼容C字符串函数
...
Redis还使用了一种叫做’定期删除（volatile-lru）'的机制来进行内存回收。它会定期地扫描一部分已过期的数据，并将这些数据从内部的数据结构中删除，并将其引用计数减1。

3. Redis还使用了一种叫做’惰性删除（lazy-free）'的机制来进行内存回收。当一个对象的引用计数变为0时，并不会立即释放该对象的内存，而是等待一个合适的时机进行释放。这样可以避免频繁的内存分配和释放，提高性能。

4. Redis的内存回收是基于引用计数的。当对象没有被引用时，通过定期删除和惰性删除机制来释放对象的内存。这种方式能够有效地回收内存，并且不会造成过多的内存碎片。


## 引用计数及对象的销毁 （一种简单的垃圾回收算法）
...
# 为什么redis使用引用计数不会造成循环引用？

在redis中引用计数法不会导致循环引用，因为ptr属性只会指向不同编码方式形成的数据结构，而不是对象。
...
1. 主线程fork一个子线程bgsave，共享主线程所有数据；  
2. 开始生成RDB文件；
3. 如果此时主线程要修改一个数据，bgsave线程继续，主线程生成修改内容的副本，并修改副本。  
...
自动触发可配置触发规则，也会在主从复制和shutdown命令时自动触发。
...
原子性atomicity  
首先通过上文知道 运行期的错误是不会回滚的，很多文章由此说Redis事务违背原子性的；Redis官方文档给的理解是，Redis的事务是原子性的：所有的命令，要么全部执行，要么全部不执行。而不是完全成功。 我觉得是不满足原子性。

一致性consistency  
在一致性的核心是约束的语意下，Redis 的事务可以保证一致性。

隔离性Isolationredis  
如果并发操作在EXEC之前，可以通过WATCH保证隔离性， 如果在EXEC之后，因为Redis是单线程的，所以也能保证隔离性。

持久性Durability  
AOF 模式的三种配置选项 no 、everysec 都会存在数据丢失的情况 。always 可以保证事务的持久性，但因为性能太差，在生产环境一般不推荐使用。
...
2. 第二阶段，主库将所有数据同步给从库。从库收到数据后，在本地完成数据加载。
...
网络断了之后，主从库会采用增量复制的方式继续同步。当主从库断连后，主库会把断连期间收到的写操作命令，写入replication buffer，同时也会把这些操作命令也写入repl_backlog_buffer这个缓冲区。
...
那客户端又是怎么知道重定向时的新实例的访问地址呢？当客户端把一个键值对的操作请求发给一个实例时，如果这个实例上并没有这个键值对映射的哈希槽，那么，这个实例就会给客户端返回MOVED命令响应结果，这个结果中就包含了新实例的访问地址。
...
keys命令可以用scan代替，每次执行都返回少量元素。但rehash扩容的过程中，scan可能会导致数据重复，重复的数据客户端去重。
...
3. 对于redis来说，主要的系统瓶颈是内存或者网络并非cpu。
...
# redis锁过期了，任务没执行完怎么处理？

定期对锁进行续期， 在获得锁成功后启动一个协程， 每隔一段时间对锁进行续期。
在续期的时候可以对比要续期的key是不是当前业务key， 如果不是则不续期。
如何服务器宕机了，续期协程也就不会运行了，锁就会自然过期。

如果服务器宕机， 只能等到超时时间后锁被自动删除。如果要立刻删除，需要增加额外的工作，比如增加哨兵机制，让哨兵来维护所有redis客户端的列表。哨兵定时监控客户端是否宕机，如果检测到宕机，立刻删除这个客户端的锁。
