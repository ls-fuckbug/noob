...
# 当前读 FOR UPDATE 会加行锁还是表锁

MySQL的FOR UPDATE语句只会对查询结果集中的行进行行锁，其他事务无法修改被锁定的行。但需要注意的是，事务隔离级别为RR时，如果没有设置正确的索引，可能会导致表级锁定而不是行级锁定。

# 更新UPDATE 会加行锁还是表锁

既可能是行锁，也可能是表锁。 取决于where条件的列有没有加索引，如果没有加索引就会导致锁表。


# Mysql为什么选择可重复读来作为默认隔离级别

"可重复读"是MySQL的默认隔离级别的原因主要是为了平衡数据的一致性和并发性能。在"可重复读"隔离级别下，一个事务在开始后会创建一个一致性视图，该视图会在整个事务期间保持一致，不受其他并发事务的修改影响。这意味着在一个事务内部，多次读取同一数据会得到一致的结果，不会出现不可重复读的情况。

通过将默认隔离级别设置为"可重复读"，MySQL可以提供较高的数据一致性。这对于很多应用场景来说是非常重要的，特别是在需要保证事务中读取到的数据的一致性和稳定性时，比如金融系统、订单处理等。同时，"可重复读"隔离级别在一定程度上可以避免幻读问题，即当一个事务在读取数据时，其他事务并发地插入新的符合条件的数据，导致该事务的结果集发生变化。

在"可重复读"隔离级别下，通过锁定读取的范围，可以避免幻读的问题。虽然"可重复读"隔离级别可以提供较高的数据一致性，但也会对并发性能造成一定的影响。因为在该隔离级别下，需要对读取的数据进行一定程度的锁定，以保证事务内部的一致性。这可能导致一些潜在的锁竞争和性能下降。
